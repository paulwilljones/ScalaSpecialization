...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): Stream[(Block, List[Move])] =
    for {
      (block, move) <- b.legalNeighbors.toStream
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    case (block, moveList) #:: tail => {
      val newNeighbors = newNeighborsOnly (neighborsWithHistory (block, moveList), explored + block)

      newNeighbors ++ from(tail ++ newNeighbors, explored + block)
    }
  }
  ...
  lazy val pathsToGoal: Stream[(Block, List[Move])] = pathsFromStart filter {
    case (block, moveList) => done(block)
  }
  ...
  lazy val solution: List[Move] = pathsToGoal match {
    ...
    case (block, moveList) #:: _ => moveList
  }
}
