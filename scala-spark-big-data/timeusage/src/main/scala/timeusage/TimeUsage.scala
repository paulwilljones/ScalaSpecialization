...
object TimeUsage {
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    columnNames.foldRight[(List[Column], List[Column], List[Column])]((Nil, Nil, Nil)) {
      (col, list) =>
        if (primary.exists(col.startsWith)) list.copy(_1 = new Column(col) :: list._1)
        else if (working.exists(col.startsWith)) list.copy(_2 = new Column(col) :: list._2)
        else if (other.exists(col.startsWith)) list.copy(_3 = new Column(col) :: list._3)
        else list
    }
  }
  ...
  def timeUsageSummary(
    ...
    val workingStatusProjection: Column =
      when(df("telfs") >= 1 && $"telfs" < 3, "working").otherwise("not working").as("working")
    val sexProjection: Column =
      when(df("tesex") === 1.0, "male").otherwise("female").as("sex")
    val ageProjection: Column =
      when(df("teage") >= 15 && $"teage" < 22, "young")
      .when(df("teage") >= 23 && $"teage" <= 55, "active")
      .otherwise("elder")
      .as("age")
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    summed
      ...
      case ((working, sex, age), primaryNeeds, work, other) => TimeUsageRow(working, sex, age,  round1(primaryNeeds),
        round1(work), round1(other))
    }.orderBy('working, 'sex, 'age)
  }
}
