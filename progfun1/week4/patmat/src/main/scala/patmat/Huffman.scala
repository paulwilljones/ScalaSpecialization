...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] =
    chars.groupBy(char => char).map(elem => (elem._1, elem._2.size)).toList
   ...
   */
    def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
      def lookup(t: CodeTree)(ch: Char): List[Bit] = t match {
        case Leaf(_, _) => List()
        case Fork(left, right, _, _) => if (chars(left).contains(ch)) 0 :: lookup(left)(ch) else 1 :: lookup(right)(ch)
      }
