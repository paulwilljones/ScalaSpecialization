 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test("sum of negative numbers") {
    assert(sum(List(-1,1,-2)) === -2)
  }

  test("sum of repeated elements") {
    assert(sum(List(1,1,1,1,1)) === 5)
  }
  ...
  test("zero return from empty list") {
    assert(sum(List()) === 0)
  }

  test("exception thrown with empty list") {
  ...
  test("max of negative numbers") {
    assert(max(List(1,2,3,4,5,6)) === 6)
  }

  test("max with negative numbers") {
    assert(max(List(-1,0,1,2,-3)) === 2)
  }

  test("max with repeat elements") {
    assert(max(List(2,2,1,1,0)) === 2)
  }
