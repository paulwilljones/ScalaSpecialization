...
object Anagrams {
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = occurrences match {
    ...
    case x :: xs => {
      ...
      val prepend = for (a <- s; b <- combinations(xs)) yield a :: b;
      prepend ++ combinations(xs)
    }
  }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def sentenceAnagramsInner(occurs: Occurrences): List[Sentence] = occurs match {
      ...
      case x :: xs => {
        ...
        for (i <- combos if dictionaryByOccurrences.keySet(i);
             j <- dictionaryByOccurrences(i);
             s <- sentenceAnagramsInner(subtract(occurs, i))) yield {j :: s}
      }
    }
